...
void BackScrool();			//背景スクロール処理
void EndBackScrool();		//背景スクロール処理(ゲームオーバー時)
...
void InputRanking(void)
{
	...
	if (KeyInputSingleCharString( 170 , 310 , 10 , g_Ranking[4].name , FALSE ) == 1){
		g_Ranking[4].score = g_Score;	// ランキングデータの4番目にスコアを登録
...
{
	EndBackScrool();
	g_Score = (g_Mileage / 10 * 10) + g_EnemyCount1 * 200;
	//スペースキーでメニューに戻る
	...
	DrawRotaGraph(230, 230, 0.3f, M_PI / 2, g_Teki[0], TRUE, FALSE);
	...
	DrawFormatString(150, 450, 0xFFFFFF, "--- スペースキーを押してタイトルへ戻る ---", 0xffffff, 0);
